...
int main(int argc, char *argv[])
{
	...
	memset(buffer, '\0', sizeof(buffer)); // Clear out the buffer array
	nread = getline(&b, &len, ciphertext); // Read from plaintext file into buffer variable
	strcpy(encipheredText, buffer); // Save buffer value to plaintext variable
	memset(buffer, '\0', sizeof(buffer)); // Clear out the buffer array
	...
	if (strcmp(receiveChar, socketName) == 0) {
		...
		stringLength = strlen(encipheredText); // Set length variable for full string

		// Slice plaintext into substring(s) with max length 1000 and send to server
		do { // Loop until last substring
			memset(sendChar, '\0', sizeof(sendChar)); // Reset sendChar variable
			sendCharLength = getSubstring(s, encipheredText); // Get substring of key with max 1000 characters
		...
		fclose(ciphertext); // Close plaintext file

		// Loop until end of plaintext found
		do {
			...
			strcat(plainText, receiveChar); // Concatenate buffer to ciphertext variable
			newline = isNewline(receiveChar); // Check for end of string
		} while(newline == 0);
		
		// Print deciphered text to standard output
		printf("%s", plainText);
		fflush(stdout);
	}
